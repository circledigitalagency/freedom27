import { GalleryItem, NavItem, Shop } from "./types";

export const menu: NavItem[] = [
	{
		label: "About us",
		path: "/about-us",
	},
	{
		label: "Our Services",
		path: "/services",
	},
	{
		label: "Gallery",
		path: "/gallery",
	},
	{
		label: "Blog",
		path: "/blog",
	},
	{
		label: "Shop",
		path: "/shop",
	},
];

export const media = [
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214255/702_ciun7x.webp",
		alt: "702 Radio Station",
	},
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214255/bbc_vzzpxg.png",
		alt: "BBC World Service",
	},
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214256/cape-pulprit_jrhajw.jpg",
		alt: "Cape Pulprit Radio Station",
	},
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214258/CGTN_Africa_fbe4za.png",
		alt: "CGTN_Africa",
	},
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214261/metro-fm_tmg7jp.png",
		alt: "Metro FM",
	},
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214261/newzroom-afrika_b0mgic.png",
		alt: "Newzroom Afrika",
	},
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214262/radio-2000_ja4nku.gif",
		alt: "Radio 2000",
	},
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214262/radio-atlatis_vfgbhx.png",
		alt: "Radio Atlatis",
	},
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214262/sunday-sun_mqt9ow.png",
		alt: "Sunday Sun",
	},
];

export const affiliates = [
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214269/alrd_dh2qs5.png",
		alt: "alrd",
	},
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214270/fnb-logo_uonjlo.webp",
		alt: "fnb",
	},
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214265/arm_vcbxrq.png",
		alt: "arm",
	},
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214271/cbrta_mrxfmm.jpg",
		alt: "cbrta",
	},
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214269/deedsoffice_a9wagi.png",
		alt: "deedsoffice",
	},
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214270/erwat_ll4pih.png",
		alt: "erwat",
	},
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214270/ewsc_jzgvtw.png",
		alt: "ewsc",
	},
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214260/gp_ciy1qx.gif",
		alt: "gp",
	},
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214273/icas_idvhgr.webp",
		alt: "icas",
	},
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214261/igt_iwg1po.gif",
		alt: "igt",
	},
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214276/inseta-logo_krzkhp.png",
		alt: "inseta",
	},
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214261/momentum_gyizbp.png",
		alt: "momentum",
	},
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214277/nifthass_yhl0xj.png",
		alt: "National Institute for the Humanities and Social Sciences",
	},
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214278/prsa_txt2lm.png",
		alt: "Parliament of the Republic of South Africa",
	},
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214278/raf_plnhft.jpg",
		alt: "Road Accident Fund",
	},
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214279/sanbi_aehaoe.png",
		alt: "Sanbi Biodiversity Advisor",
	},
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214280/tax-ombud_thsatx.png",
		alt: "Tax Ombud",
	},
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214262/transnet_kj5gcl.png",
		alt: "Transnet",
	},
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214263/trsa_pj9ml6.jpg",
		alt: "Tourism Republic of South Africa",
	},
	{
		src: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214280/wcg_gt5mff.png",
		alt: "Western Cape Government",
	},
];

export const approach = [
	{
		id: 1,
		img: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751218997/mental-clarity_1_jekuo0.jpg",
		title: "Mental clarity techniques",
		subheading: "Focus sessions & Limiting belief reprogramming",
	},
	{
		id: 2,
		img: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751359903/trauma-healing_1_ak823a.jpg",
		title: "Stress and trauma healing",
		subheading: "Forgiveness & Emotional release",
	},
	{
		id: 3,
		img: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1752151536/growth-strategies_1_lhywpv.jpg",
		title: "Personal growth strategies",
		subheading: "Goal setting, Motivation & Gratitude",
	},
	{
		id: 4,
		img: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214294/subconscious-healing_g3bymp.jpg",
		title: "Subconscious mind coaching",
		subheading: "Undersding patterns and triggers",
	},
	{
		id: 5,
		img: "https://res.cloudinary.com/dg1g6ctku/image/upload/v1752151543/mental-health_1_ite11o.jpg",
		title: "Mental health awareness",
		subheading: "Trauma release & Self-awareness",
	},
];

export const experiences = [
	{
		id: 1,
		icon: "/images/life-coaching01.png",
		label: "One on One Life Coaching",
		value: [
			"Personalized sessions to support your growth, clarity, and emotional well-being, guided by compassionate listening and transformative tools.",
		],
		takeaways:
			"Stress reduction, clarity, physical wellness, emotional grounding",
	},
	{
		id: 2,
		icon: "/images/couple-session01.png",
		label: "Couples session",
		value: [
			"Strengthen connection, communication, and emotional harmony in your relationship through mindful, heart-centered guidance.",
		],
		takeaways:
			"Stress reduction, clarity, physical wellness, emotional grounding",
	},
	{
		id: 3,
		icon: "/images/hikes.png",
		label: "Hikes & Outdoor Mindfulness Experiences",
		value: [
			"Guided hikes with intention-setting, breathing exercises, and focus techniques",
			"Group discussions on mindfulness, resilience, and emotional well-being",
		],
		takeaways:
			"Stress reduction, clarity, physical wellness, emotional grounding",
	},
	{
		id: 4,
		icon: "/images/workshop.png",
		label: "1-Day Workshop",
		value: [
			"A deep dive into mindfulness, personal growth, and self-awareness",
			"Gratitude practices, limiting beliefs, motivation, trauma healing",
		],
		takeaways:
			"Mental resilience, stress management tools, personal goal setting",
	},
	{
		id: 3,
		icon: "/images/1day-group-session.png",
		label: "1-Day Group Session(Camp or On-site)",
		value: [
			"Mindfulness training, group coaching, focus exercises, trauma release",
			"Outdoor setting for relaxation and immersive learning",
		],
		takeaways:
			"Emotional clarity, improved concentration, relaxation techniques",
	},
	{
		id: 4,
		icon: "/images/2day-group-session.png",
		label: "2-Day Group Session(Camp or On-site)",
		value: [
			"Day 1: Mindfulness & Emotional Release",
			"Day 2: Behavioral Coaching & Practical Application",
			"Yoga, guided meditations, group discussions",
		],
		takeaways:
			"Self-awareness, emotional intelligence, practical mental health tools",
	},
	{
		id: 5,
		icon: "/images/retreat.png",
		label: "3-Day individual or Group (Retreat Experience)",
		value: [
			"In-depth self-discovery, subconscious reprogramming, group coaching",
			"Personalized coaching sessions with Tshepo Matlou and experienced mindfulness experts",
		],
		takeaways: "Healing, clarity, strategic life transformation",
	},
	{
		id: 6,
		icon: "/images/networking.png",
		label: "Breakfast & Networking Sessions",
		value: [
			"A more casual but impactful coaching environment",
			"Mindfulness for business leaders, stress management, mental clarity",
		],
		takeaways:
			"Professional and personal alignment, motivation, leadership growth",
	},
	{
		id: 7,
		icon: "/images/masterclasses.png",
		label: "Masterclasses & Specialized Trainings",
		value: [
			"A more casual but impactful coaching environment",
			"Mindfulness for business leaders, stress management, mental clarity",
		],
		takeaways:
			"Focused on mindset reprogramming, trauma healing, leadership coaching, and behavioural training",
	},
];

export const blogPosts = [
	{
		title: "Trauma Healing",
		excerpt:
			"Trauma is a disturbing experience that could have occurred during childhood, loss of life, abuse in multiple forms, relationship, work environment and many more events. It can manifest itself as an emotional, physical or psychological harm.",
		blogText:
			"<p><strong>What is trauma?</strong></p> <p>Trauma is a disturbing experience that could have occurred during childhood, loss of life, abuse in multiple forms, relationship, work environment and many more events. It can manifest itself as an emotional, physical or psychological harm.</p> <br /> <p>Reliving our traumatic experiences without healing creates a stressful mental state, and dependency which can lead to becoming easily irritable, wondering mind, attracting the wrong people, aging faster and become more frustrated. There must be an intention to heal from the things that we experienced that are self-sabotaging. The good news is you can recover from traumatic experiences through a self-healing process.</p> <br /> <p>Here's an interesting perspective, whenever you are feeling sick, your body has identified elements that are not meant to be in your well functional structure. This leads to your body rejecting toxic components you have consumed or that have developed in order for you to get to a point where you feel better. But guess what, if your body doesn’t have the capacity to heal and fight those toxins, you may die unless you seek help immediately. Same applies with your traumas, if you leave them unattended, you are at constant risk of emotional and mental death before the physical death. But if you take your conscious time to work through them, you increase your chances of enjoying the best version of yourself.</p> <br /> <p>Healing from a trauma is a journey that requires patience, self-compassion, and support. By acknowledging your traumatic experiences and working through them diligently, you can reclaim your life and build a stronger, more resilient future. Remember, you are not alone, we are with you and there are resources available to support and empower you every step of the way.</p> <br />",
		author: "Tshepo Matlou",
		date: "July 07, 2025",
		readTime: "5 min read",
		image:
			"https://res.cloudinary.com/dg1g6ctku/image/upload/v1752150769/william-farlow-IevaZPwq0mw-unsplash_vygvrp.jpg",
		category: "Healing",
	},
	{
		title: "Overcoming Anxiety: A Journey to your Freedom",
		excerpt:
			"Are you tired of feeling anxious and overwhelmed? Do you struggle to manage your worries and fears? Are you uncertain about the future? You're not alone. Anxiety is a common experience that can affect anyone, regardless of their background or circumstances and it comes in different forms and types.",
		blogText:
			"<p>Are you tired of feeling anxious and overwhelmed? Do you struggle to manage your worries and fears? Are you uncertain about the future? You're not alone. Anxiety is a common experience that can affect anyone, regardless of their background or circumstances and it comes in different forms and types.</p> <br /> <p>Are you tired of feeling anxious and overwhelmed? Do you struggle to manage your worries and fears? Are you uncertain about the future? You're not alone. Anxiety is a common experience that can affect anyone, regardless of their background or circumstances and it comes in different forms and types. </p> <br /> <p>The potential of an undesired outcome grips us in ways that leaves us emotionally devastated and unregulated. It gives us worry and anxiety. When we grow old without achieving what we wanted, we become anxious of the future and what it holds. Our world and perspectives become chaotic.</p> <br /> <p>When you are overly anxious, you lose clarity of the present moment and are deprived of the opportunity to be yourself. It’s never too late to restart life, at whatever age you are; for as long as you are still alive you can start again.</p> <br /> <p>Tshepo Matlou found from his traumatic life experience and research that to be free is to be present. Meaning you can never fully utilise what you have now because your focus is on what you do not have or on how things could have been. The beauty of life lies not in the depression of the past or the anxiety of the future, but in the gift of the present moment.</p> <br /> <p>At Freedom 27, we understand the impact of anxiety on your life, and we’re here to help. We can hold your hand to your desired freedom, clarity and confidence. We can support you and be with you every step of the way. We have personal and professional tools & frameworks that work for your unique needs. Collaborate with a partner that understands you.</p> <br /> <p><i>Freedom 27</i>, where we journey to your freedom.</p> <br />",
		author: "Tshepo Matlou",
		date: "July 07, 2025",
		readTime: "5 min read",
		image:
			"https://res.cloudinary.com/dg1g6ctku/image/upload/v1752150742/annie-spratt-H582qBzGmHQ-unsplash_kolxif.jpg",
		category: "Mindfulness",
	},
	{
		title: "Your depression is lying to you",
		excerpt:
			"Did you know that your depression is lying to you? Did you know that whatever situation you are going through; you have the capacity to overcome and bring life changing solutions to it?",
		blogText:
			"<p>Did you know that your depression is lying to you? Did you know that whatever situation you are going through; you have the capacity to overcome and bring life changing solutions to it?</p> <br /> <p>With life’s struggle and suffering that is nested within human existence lies the potential for one to deteriorate into depression. This depression overly exaggerates how bad your situations and emotions are, which unfortunately lead others into committing suicide. It goes without saying that depression has its clinical diagnosis that emphasizes it’s realness, and acknowledging that reality doesn’t mean that the depression is the truth you should be living in.</p> <p><strong>For that depression is lying to you.</strong></p> <br /> <p>The truth is that what you feel depressed about is in the past and as real as those feelings are to you, the reality is in the past. You need to have enough self compassion to create a reality that is far and better from what depressed you. You can still live a life of purpose, meaning and fulfillment, only if you stop living in the past. Only if you stop feeling sorry for yourself and take centre stage in creating a life of freedom. There has to be an intention to forgive those who do not ask for forgiveness, to release them from our system.</p> <br /> <p>You need to have a will to fight for your freedom from your depression. Albert Einstein said Imagination is more powerful than knowledge. Make sure that the nation in your mind is positive, it speaks highly of you, continues to think about solutions for every problem you are facing. And in so doing, you have embarked on your depression - free journey and on enjoying days worth living a fulfilled life.</p> <br /> <p>Join as we embark in healing solutions which are connect to nature, through fire talks, nature hikes and connecting spaces to find mechanism to cope and survive but beyond these be Free.</p> <br /> <p>Take a step and partner with us.</p>",
		author: "Tshepo Matlou",
		date: "Dec 14, 2024",
		readTime: "6 min read",
		image:
			"https://res.cloudinary.com/dg1g6ctku/image/upload/v1752150766/k-mitch-hodge-IqSaG9zv2e0-unsplash_thk0pc.jpg",
		category: "Healing",
	},
];

export const shopData: Shop[] = [
	{
		id: "book",
		image:
			"https://res.cloudinary.com/dg1g6ctku/image/upload/v1751621135/book_hexyzz.svg",
		name: "Freedom - Pursuit of Liberty",
		description:
			"Discover the strength in surrender. This book guides you through healing old wounds, embracing change, and finding peace through mindfulness and self-awareness.",
		price: 350,
	},
	// {
	// 	id: "oneOnone",
	// 	image:
	// 		"https://res.cloudinary.com/dg1g6ctku/image/upload/v1751214294/subconscious-healing_g3bymp.jpg",
	// 	name: "One on One Life Coaching",
	// 	description:
	// 		"Personalized sessions to support your growth, clarity, and emotional well-being — guided by compassionate listening and transformative tools.",
	// 	packages: [
	// 		{
	// 			id: "4sessions",
	// 			name: "4 Sessions",
	// 			price: 1200,
	// 		},
	// 	],
	// },
	// {
	// 	id: "couple",
	// 	image: "/images/coaching02.jpg",
	// 	name: "Couples session",
	// 	description:
	// 		"Strengthen connection, communication, and emotional harmony in your relationship through mindful, heart-centered guidance.",
	// 	packages: [
	// 		{
	// 			id: "4sessions",
	// 			name: "4 Sessions",
	// 			price: 1800,
	// 		},
	// 	],
	// },
];

export const values = [
	"Empowerment",
	"Authenticity",
	"Courage",
	"Resilience",
	"Freedom",
];

export const qualifications = [
	{
		degree: "Masters in Leadership & Management",
		school: "",
	},
	{
		degree: "Management Advanced Programme",
		school: "Wits Business School",
	},
	{
		degree: "Advanced Public Management Development Programme",
		school: "University of Pretoria",
	},
	{
		degree: "Diploma in Marketing Management",
		school: "Tshwane College",
	},
	{
		degree: "Executive and Management Coaching",
		school: "University of Cape Town",
	},
	{
		degree: "Mastery Coaching",
		school: "(ISNS); Team Coaching – Meta Coaching Foundation",
	},
	{
		degree: "Mauritius Mentoring and Coaching for Public Managers",
		school: "School of Government",
	},
	{
		degree: "Strategies for Personal Growth and Development",
		school: "YALI",
	},
	{
		degree: "Management Strategies for People and Resources",
		school: "YALI",
	},
	{
		degree: "Prince2 and PMBOK",
		school: "",
	},
];

export const galleryItems: GalleryItem[] = [
	{
		id: 1,
		title: "Freedom 27 Book Launch",
		image:
			"https://res.cloudinary.com/dg1g6ctku/image/upload/v1750939353/IMG_5103_sfs7ui.jpg",
		category: "Book Launch",
	},
	{
		id: 2,
		title: "Freedom 27 Book Launch",
		image:
			"https://res.cloudinary.com/dg1g6ctku/image/upload/v1750939424/IMG_8630_sgcagi.jpg",
		category: "Book Launch",
	},
	{
		id: 3,
		title: "Women's Month Celebration",
		image:
			"https://res.cloudinary.com/dg1g6ctku/image/upload/f_jpg/v1750939333/FullSizeRender_om7ffu.heic",
		category: "Talks",
	},
	{
		id: 4,
		image:
			"https://res.cloudinary.com/dg1g6ctku/image/upload/v1750939315/3370545b-1a54-449e-b0c0-657e63cc1294_cueo1l.jpg",
		category: "Talks",
	},
	{
		id: 5,
		image:
			"https://res.cloudinary.com/dg1g6ctku/image/upload/v1750939391/IMG_6891_mrqnpc.jpg",
		category: "Talks",
	},
	{
		id: 6,
		image:
			"https://res.cloudinary.com/dg1g6ctku/image/upload/f_jpg/v1750939383/IMG_7318_hzq7tq.heic",
		category: "Talks",
	},
	{
		id: 7,
		image:
			"https://res.cloudinary.com/dg1g6ctku/image/upload/f_jpg/v1750939388/IMG_7365_bic4kw.heic",
		category: "Talks",
	},
	{
		id: 8,
		image:
			"https://res.cloudinary.com/dg1g6ctku/image/upload/f_jpg/v1750939359/IMG_2261_hmflif.heic",
		category: "Workshops",
	},
	{
		id: 9,
		image:
			"https://res.cloudinary.com/dg1g6ctku/image/upload/f_jpg/v1750939358/IMG_6520_o002pg.heic",
		category: "Book",
	},
	{
		id: 10,
		image:
			"https://res.cloudinary.com/dg1g6ctku/image/upload/f_jpg/v1750939357/IMG_5878_t66qxw.heic",
		category: "Radio",
	},
	{
		id: 11,
		image:
			"https://res.cloudinary.com/dg1g6ctku/image/upload/f_jpg/v1750939355/IMG_5874_ci3ntu.heic",
		category: "Radio",
	},
	{
		id: 12,
		image:
			"https://res.cloudinary.com/dg1g6ctku/image/upload/f_jpg/v1750939355/IMG_4693_isvthe.heic",
		category: "Talks",
	},
	{
		id: 13,
		image:
			"https://res.cloudinary.com/dg1g6ctku/image/upload/v1750939318/FB_IMG_1507614248584_guatmq.jpg",
		category: "Workshops",
	},
	{
		id: 14,
		image:
			"https://res.cloudinary.com/dg1g6ctku/image/upload/v1750939337/IMG_1830_gwlrcv.jpg",
		category: "Workshops",
	},
	{
		id: 15,

		image:
			"https://res.cloudinary.com/dg1g6ctku/image/upload/v1750939334/IMG_1828_z4fskt.jpg",
		category: "Workshops",
	},
	{
		id: 16,
		image:
			"https://res.cloudinary.com/dg1g6ctku/image/upload/v1750939315/FB_IMG_1507614241875_qwyhni.jpg",
		category: "Workshops",
	},
	{
		id: 17,
		image:
			"https://res.cloudinary.com/dg1g6ctku/image/upload/v1750939314/87cad0b3-10d4-44a1-8f1b-186eb2ce48e4_mp1ikh.jpg",
		category: "Workshops",
	},
	{
		id: 13,
		image:
			"https://res.cloudinary.com/dg1g6ctku/image/upload/v1750939314/5b968995-4b26-48c4-98d3-cc1ddb2c8773_ossjfw.jpg",
		category: "Workshops",
	},
	{
		id: 14,
		image:
			"https://res.cloudinary.com/dg1g6ctku/image/upload/v1750939417/SAM_0800_beu8z3.jpg",
		category: "Talks",
	},
	{
		id: 15,
		image:
			"https://res.cloudinary.com/dg1g6ctku/image/upload/v1750939314/431fd537-60db-48db-9e4d-540fce555418_waljfs.jpg",
		category: "Talks",
	},
	{
		id: 16,
		image:
			"https://res.cloudinary.com/dg1g6ctku/image/upload/v1750939389/IMG_7342_d8yea5.jpg",
		category: "Hiking",
	},
	{
		id: 17,
		image:
			"https://res.cloudinary.com/dg1g6ctku/image/upload/v1750939363/IMG_6685_f9xh0o.jpg",
		category: "Workshops",
	},
];
